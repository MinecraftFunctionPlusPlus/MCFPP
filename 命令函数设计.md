...
`CommandValue`是几乎所有命令函数的返回值类型。它有两个成员，`result`和`success`，对应命令是否执行成功，或者命令执行的返回值。
...
此命令需要以玩家为对象。此后内容略去`Player.advancements`
...
* `attribute["xxx"].value`获得xxx属性的值
* `attribute["xxx"].set(float value)`设置属性的base值
