...
//    }
    ...
    companion object {
        ...
        private val typeCache: MutableMap<String, MCFPPType> by lazy {
            mutableMapOf(
            ...
            if (ctx.className() != null) {
                ...
                if (clazz != null) {
                    ...
                    if (clazz is GenericClass) {
                        if (clazz.readOnlyParams.size != ctx.type().readOnlyArgs()?.expressionList()
                                ?.expression()?.size
                        ) {
                            LogProcessor.error(
                                "Generic class ${clazz.identifier} requires ${clazz.readOnlyParams.size} type arguments, but ${
                                    ctx.readOnlyArgs().expressionList().expression().size
                                } were provided"
                            )
                            return MCFPPBaseType.Any
                        }
                        ...
                        val readOnlyArgs =
                            ctx.type().readOnlyArgs()?.expressionList()?.expression()?.map { expr.visit(it)!! }
                                ?: listOf()
