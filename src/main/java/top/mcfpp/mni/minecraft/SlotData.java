...
public class SlotData {
    ...
    @MNIFunction(caller = "Slot", returnType = "JavaVar", override = true)
    public static void toCommandPart(DataTemplateObject caller, ValueWrapper<JavaVar> returnValue){
        ...
        Command command = new Command().build(type.toCommandPart(), false);
        if(index.getHasAssigned()){
            command.build(".", false).build(index.toCommandPart(), false);
        }
